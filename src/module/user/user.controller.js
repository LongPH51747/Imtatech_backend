...
exports.changeAvata = async (req, res) => {
  try {
    const { avatar } = req.body;
    const userId = req.user._id;
    const avataUser = await userService.changeAvata(avatar, userId);
    if (!avataUser) {
      res.status(404).json({ message: "Cannot found to avatar" });
    }
    return res
      .status(200)
      .json({ message: "Cập nhật avatar thành công", user: updatedUser });
  } catch (error) {
